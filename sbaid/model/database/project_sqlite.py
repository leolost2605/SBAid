...
class ProjectSQLite(ProjectDatabase):
    ...
    async def open(self, file: Gio.File) -> None:
        ...
        Also, insert a single row into the meta-data table that can contain the meta-information
        about the project that this database represents."""
...
CREATE TABLE parameter_tag (
    ...
    cross_section_id TEXT,
    tag_id TEXT,
    PRIMARY KEY (parameter_name, algorithm_configuration_id, cross_section_id, tag_id),
    FOREIGN KEY (parameter_name, cross_section_id, tag_id) REFERENCES parameter(name, cross_section_id, tag_id) ON DELETE CASCADE,
    FOREIGN KEY (tag_id) REFERENCES tag(id) ON DELETE CASCADE
);""")
            await db.commit()
            await db.execute("""
        INSERT INTO meta_information (name, created_at, last_modified) VALUES
        (?, ?, ?)""", ("", GLib.DateTime.format_iso8601(GLib.DateTime.new_now_local()),
    ...
    async def get_created_at(self) -> GLib.DateTime:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            async with db.execute("""SELECT created_at
                ...
                return GLib.DateTime.new_from_iso8601(date[0])
    ...
    async def get_last_modified(self) -> GLib.DateTime:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            async with db.execute("""SELECT last_modified
                ...
                date = await cursor.fetchone()
                if date is None:
                    raise KeyError("Invalid last modified time.")
                return GLib.DateTime.new_from_iso8601(date[0])
    ...
    async def get_selected_algorithm_configuration_id(self) -> str:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            async with db.execute("""
        SELECT id FROM algorithm_configuration WHERE is_selected = 1""") as cursor:
                result = await cursor.fetchone()
                if result is None:
                    return ""
                return list(result)[0]
    ...
    async def get_all_parameters(self, algorithm_configuration_id: str) -> list[tuple[str, str]]:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            async with db.execute("""
            SELECT parameter_name, cross_section_id FROM parameter
            WHERE algorithm_configuration_id = ?""", [algorithm_configuration_id]) as cursor:
    ...
    async def set_parameter_value(self, algorithm_configuration_id: str, parameter_name: str,
        ...
        async with aiosqlite.connect(self._db_path) as db:
            if cross_section_id is None:
                await db.execute("""
                UPDATE parameter SET value = ? WHERE algorithm_configuration_id = ? AND name = ?
                AND cross_section_id IS NULL""", (parameter_value.print_(True), algorithm_configuration_id, parameter_name))
            else:
                await db.execute("""
            UPDATE parameter SET value = ? WHERE algorithm_configuration_id = ?
            AND name = ? AND cross_section = ?""", (parameter_value.print_(True), algorithm_configuration_id,
    ...
    async def add_parameter(self, algorithm_configuration_id: str, name: str,
        ...
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute("""
            INSERT INTO parameter (algorithm_configuration_id, name, cross_section_id, value)
            VALUES (?, ?, NULL, ?)""", (algorithm_configuration_id, name, value.print_(True)))
            await db.commit()
    ...
    async def remove_parameter(self, algorithm_configuration_id: str, name: str,
        ...
        async with aiosqlite.connect(self._db_path) as db:
            if cross_section_id is None:
                await db.execute("""
                DELETE FROM parameter WHERE algorithm_configuration_id = ? AND name = ?
                AND cross_section_id IS NULL""", (algorithm_configuration_id, name))
            else:
                await db.execute("""
                DELETE FROM parameter WHERE algorithm_configuration_id = ? AND name = ?
                AND cross_section_id = ?""", (algorithm_configuration_id, name, cross_section_id))
            await db.commit()
    ...
    async def add_parameter_tag(self, parameter_tag_id: str, parameter_name: str,
        ...
        """Add a new parameter tag entry which represents a tag belonging to the given parameter."""
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute("""
        INSERT INTO parameter_tag (parameter_name, algorithm_configuration_id,
        cross_section_id, tag_id) VALUES (?, ?, ?, ?)""", (parameter_name,
                                                        algorithm_configuration_id,
                                                        cross_section_id, tag_id))
            await db.commit()
    ...
    async def remove_parameter_tag(self, parameter_tag_id: str) -> None:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute("""
        DELETE FROM parameter_tag WHERE parameter_tag_id = ?""", [parameter_tag_id])
            await db.commit()

    async def get_all_tag_ids_for_parameter(self, algorithm_configuration_id: str,
                                     parameter_name: str, cross_section_id: str | None) -> list[str]:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            if cross_section_id is None:
                ...
                AND cross_section_id IS NULL""", (parameter_name, algorithm_configuration_id)) as cursor:
                ...
                async with db.execute("""SELECT tag_id FROM parameter_tag
                WHERE algorithm_configuration_id = ? AND parameter_name = ? AND cross_section_id = ?""",
                                      (algorithm_configuration_id, parameter_name,
                                       cross_section_id))as cursor:
                    result_cursor = await cursor.fetchall()
                    result = map(lambda x: x[0], result_cursor)
