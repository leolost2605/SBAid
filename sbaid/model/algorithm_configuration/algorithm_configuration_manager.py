...
class AlgorithmConfigurationManager(GObject.GObject):
    ...
    def __init__(self, network: Network) -> None:
        """Constructs an algorithm configuration manager instance."""
        super().__init__(network=network)
        self.algorithm_configurations = Gio.ListModel().__init__()
        self.network = network

    def load(self) -> None:
        """Loads the algorithm configurations and adds them to the list."""
    ...
    def create_algorithm_configuration(self) -> int:
        """Creates a new algorithm configuration whose position in the list is
        then returned."""
