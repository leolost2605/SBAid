...
async def generate_result(snapshot_amount: int, cs_amount: int, lane_amount: int) -> Result:
    ...
    result_builder = ResultBuilder(ResultManager(unittest.mock.AsyncMock()))
    result_builder.begin_result("randomized_result")
    random_ids = __generate_random_ids(cs_amount)
...
def __generate_random_ids(n: int):
    random_ids = []
    for i in range(n):
