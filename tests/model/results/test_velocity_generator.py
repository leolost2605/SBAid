...
class VelocityGeneratorTest(unittest.TestCase):
    ...
    def generate_random_result(self) -> Result:
        ...
        global_placeholder_db = GlobalSQLite(gio_file)
        result = Result(test_id, test_name, now, global_placeholder_db)

        for i in range(50):
            minute = (30 + i) % 60
            hour = 15 + ((30 + i) // 60)
            time = GLib.DateTime.new(tz=GLib.TimeZone.new_local(), year=2025, month=7, day=24, hour=hour, minute = minute, seconds=0)
            snapshot = Snapshot(GLib.uuid_string_random(), time, global_placeholder_db)
            for m in range(10):
                if m == 4:
                ...
                for p in range(6):
                    lane_snapshot = lane_snapshot = LaneSnapshot(GLib.uuid_string_random(),
                                     GLib.uuid_string_random(), p,
                                       random.uniform(60, 140), random.randint(5, 30), random.choice(list(ADisplay)), global_placeholder_db)
